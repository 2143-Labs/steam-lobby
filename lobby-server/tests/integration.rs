...
// ── stats API (GET /api/leaderboard/{game}, GET /api/player/{id}) ─────────

#[sqlx::test]
async fn api_leaderboard_orders_by_rating(pool: sqlx::PgPool) {
    let h = setup(pool.clone()).await;
    sqlx::query("INSERT INTO users (steam_id, display_name) VALUES (9001, 'alice'), (9002, 'bob')")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query(
    ...
    // Ordered by rating (mu - 3*sigma) descending: alice 35-15=20, bob 25-15=10.
    assert_eq!(rows[0]["display_name"], "alice");
    assert_eq!(rows[0]["rating"], 20.0);
    assert_eq!(rows[1]["display_name"], "bob");
    assert_eq!(rows[1]["rating"], 10.0);
    assert!(rows[0]["player_id"].as_str().unwrap().len() == 36, "uuid string");

    // Unknown game_mode → 404.
    let resp = client
        .get(h.base_url.clone() + "/api/leaderboard/does_not_exist")
        .send()
        .await
        .unwrap();
...
#[sqlx::test]
async fn api_player_profile_returns_ratings_identities_history(pool: sqlx::PgPool) {
    let h = setup(pool.clone()).await;
    sqlx::query("INSERT INTO users (steam_id, display_name) VALUES (9001, 'alice'), (9002, 'bob')")
        .execute(&pool)
        .await
    ...
    sqlx::query(
        "INSERT INTO ratings (user_id, game_mode, mu, sigma) \
         SELECT id, 'rps_1v1', 25, 4 FROM users WHERE steam_id = 9001",
    )
    .execute(&pool)
    .await
    .unwrap();
    ...
    sqlx::query(
        "INSERT INTO match_results (match_token, outcome, mu_change_a, mu_change_b) \
         VALUES ('m1', 'Win', 2.1, -2.1)",
    )
    .execute(&pool)
    .await
    .unwrap();

    let alice: String = sqlx::query_scalar("SELECT id::text FROM users WHERE steam_id = 9001")
        .fetch_one(&pool)
        .await
        .unwrap();
    let bob: String = sqlx::query_scalar("SELECT id::text FROM users WHERE steam_id = 9002")
        .fetch_one(&pool)
        .await
        .unwrap();

    let client = reqwest::Client::new();
    // alice (player_a): outcome stays Win, mu_change is hers (2.1).
    let resp = client
        .get(h.base_url.clone() + "/api/player/" + &alice)
        .send()
        .await
        .unwrap();
    assert_eq!(resp.status(), reqwest::StatusCode::OK);
    let p: serde_json::Value = resp.json().await.unwrap();
    assert_eq!(p["display_name"], "alice");
    assert_eq!(p["primary_provider"], "steam");
    assert_eq!(p["identities"][0]["provider"], "discord");
    assert!(
        p["identities"][0].get("provider_uid").is_none(),
        "provider_uid must never be exposed"
    );
    let ratings = p["ratings"].as_array().unwrap();
    assert_eq!(ratings.len(), 2);
    let rps = ratings
        .iter()
        .find(|r| r["game_mode"] == "rps_1v1")
        .unwrap();
    assert_eq!(rps["rating"], 13.0); // 25 - 3*4
    let matches = p["recent_matches"].as_array().unwrap();
    assert_eq!(matches.len(), 1);
    assert_eq!(matches[0]["outcome"], "Win");
    assert_eq!(matches[0]["mu_change"], 2.1);
    assert_eq!(matches[0]["opponent_id"], bob);

    // bob (player_b): the stored Win flips to Loss; mu_change is his (-2.1).
    let resp = client
        .get(h.base_url.clone() + "/api/player/" + &bob)
        .send()
        .await
        .unwrap();
    assert_eq!(resp.status(), reqwest::StatusCode::OK);
    let p: serde_json::Value = resp.json().await.unwrap();
    let matches = p["recent_matches"].as_array().unwrap();
    assert_eq!(matches[0]["outcome"], "Loss");
    assert_eq!(matches[0]["mu_change"], -2.1);
    assert_eq!(matches[0]["opponent_id"], alice);

    // Unknown / malformed player ids → 404.
    let resp = client
        .get(h.base_url.clone() + "/api/player/00000000-0000-0000-0000-000000000000")
        .send()
        .await
        .unwrap();
    assert_eq!(resp.status(), reqwest::StatusCode::NOT_FOUND);
    let resp = client
        .get(h.base_url.clone() + "/api/player/not-a-uuid")
        .send()
        .await
        .unwrap();
    assert_eq!(resp.status(), reqwest::StatusCode::NOT_FOUND);
