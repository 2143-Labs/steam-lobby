...
**Known issue (2026-08-07):** the k3s node nftables blocks inbound UDP to the NodePort range. The LoadBalancer IP `192.168.6.14` is not routable from the workstation LAN (MetalLB BGP route not propagated to the office subnet). Until the firewall is opened or the BGP route is fixed, STUN/TURN testing from the LAN workstation will fail. Test from within the cluster using a debug pod on the same node, or add the runtime iptables rule documented in section 6.
